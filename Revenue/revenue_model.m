...
function [netRevenue,annualCC,startCost,annualOM,totalCC,fixed_om] = revenue_model(TECH,object,...
    T_DISCHARGE,POWER,ENERGY,MAIN_POWER,MIN_LOAD,life,interest,period,peakAmplitude,...
...
%     DP   = ADP-ACP ;            % $/MWh, delta price
    ...
    disp('Output from revenue\_model.m:')
    
    totalCC    = Cp_scaled+Ce_scaled;
    disp(['Total overnight CC is ' num2str(totalCC) ' MM$'])
    
    annualCC   = totalCC*(interest+(interest/((1+interest)^life-1))); % amortized capital cost
    disp(['Amortized CC is ' num2str(annualCC) ' MM$/year'])
    
    fixed_om   = (Oe_scaled+Op_scaled)*(10^6)/(POWER*1000);
    disp(['Fixed O&M is ' num2str(fixed_om) ' $/kW-year'])
    
    annualOM   = Op_scaled + Oe_scaled + cyclingCost + var_om*ENERGY*Y/1000000;
    disp(['Total O&M is ' num2str(annualOM) ' MM$/year'])
    ...
    netRevenue = RD-RC-annualCC-annualOM;               % revenue from addition of accumulator
